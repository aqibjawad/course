...
const Semester = () => {
    ...
    const submit = async (event) => {
        ...
        POST("semester", formData).then((res) => {
    ...
    const edit = async (event, id) => {
        GETID("semester", id, "").then((result)=>{
            setSemesterId(result[0]);
        });
        handleShow();
    };
    ...
    const eSubmit = (event, id) => {
        ...
        PUT("semester", id, formData).then((res) => {
    ...
    const [semesters, setSemesters] = useState(false);
    ...
    const remove = async (event, id) => {
        await DELETE("semester/delete", id, "");
        fetchData();
    };
       ...
       <Row>
                                ...
                                <tbody>
                                    {semesters && semesters.map((semester) => (
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <AiFillDelete style={{ color: 'red' }} onClick={(e) => remove(e, semester.id)} />
                                                <BsFillPencilFill style={{ color: 'blue', marginLeft: '1rem' }} onClick={(e) => edit(e, semester.id)} />
                                            </td>
                                        </tr>
                                    ))}
            ...
            <Modal show={show} onHide={handleClose}>
                ...
                <Modal.Body>
                    <Row className="">
                        <Col md={6}>
                            <Form.Label htmlFor="basic-url" style={{ fontFamily: "serif", fontWeight: "normal" }}> Category </Form.Label>
                            <Form.Control ref={edepartmentRef} className="form-control" as="select" style={{background:"#E6E6E6"}} >
                                <option value={semesterid.department}>   </option>
                                {departments.map((department) => (
                                    <option value={department.id}>{department.department}</option>
                                ))}
                            </Form.Control>
                        ...
                        <Col md={6}>
                            ...
                            <Form.Control ref={esessionRef} className="form-control" as="select"style={{background:"#E6E6E6"}} placeholder={semesterid.session}>
                                <option value=""> --- Select --- </option>
                                {sessions.map((session) => (
                                    <option value={session.id}>{session.session}</option>
                                ))}
                        ...
                        <Col md={6}>
                            <Form.Group className="">
                                <Form.Label style={{ fontFamily: "serif", fontWeight: "normal" }}>   Section </Form.Label>
                                <Form.Control ref={esemesterRef} className="form-control" as="select" style={{background:"#E6E6E6"}} placeholder={semesterid.semester}>
                                    <option value=""> --- Select --- </option>
                                    <option value="1"> 1</option>
                                    <option value="2"> 2</option>
                                    <option value="3"> 3</option>
                ...
                <div className="my-2 pl-3 mb-5">
                    <Button className="btn-custom border-0 mx-3" variant="danger" onClick={(e) => eSubmit(e, semesterid)} style={{ fontFamily: "serif", fontWeight: "normal" ,background:"rgba(7,48,111,1)", borderColor:"rgba(7,48,111,1)"}}>
                        Modify
                    </Button>
                </div>
            </Modal>
